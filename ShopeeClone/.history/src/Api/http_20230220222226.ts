...
class Http {
  ...
  constructor() {
    this.accessToken = WorkingWithLS.getAccessTokenFromLS()
    ...
    this.instance.interceptors.response.use(
      (response: AxiosResponse<ResponveApi<AthDataResponve>>) => {
        ...
        if (url && [PathRoute.login as string, PathRoute.register as string].includes(url)) {
          ...
          WorkingWithLS.saveAccesTokenToLS(response.data.data.access_token)
        } else if (url === PathRoute.logout) {
          this.accessToken = ''
          WorkingWithLS.clearAccessTokenFromLS()
        }
        return response
      },
