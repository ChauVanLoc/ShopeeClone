...
class Http {
  ...
  constructor() {
    this.accessToken = WorkingWithLS.getAccessTokenFromLS('access_token')
    ...
    this.instance.interceptors.response.use(
      (response: AxiosResponse<ResponveApi<AthDataResponve>>) => {
        ...
        if (url && [PathRoute.login as string, PathRoute.register as string].includes(url)) {
          ...
          WorkingWithLS.saveToLS([{ key: 'access_token', value: this.accessToken }])
        } else if (url === PathRoute.logout) {
          this.accessToken = ''
          WorkingWithLS.clearFromLS(['access_token'])
        }
        return response
      },
