...
function Product({ product, classNameBlock }: ProductProps) {
  return (
    <Link to='' className={classNameBlock}>
      <img className='max-w-full' src={product.image} alt='img' />
      <div className='p-2'>
        ...
        <div className='mt-3 mb-5 flex'>
          <div className='mr-3 text-gray-300 line-through'>
            <span>₫</span>
            <span>{convertCurrentcy(product.price_before_discount)}</span>
          </div>
        ...
        <div className='flex'>
          ...
          <div className='ml-3 text-[11px]'>Đã bán ${convertDigitalNumber(product.sold)}k</div>
        </div>
      </div>
    </Link>
  )
