...
function Product({ product, classNameBlock, rating }: ProductProps) {
  const description = useMemo(() => ['terrible', 'bad', 'normal', 'good', 'wonderful'], [])
  const [value, setvalue] = useState<number>(product.rating)
  return (
    <Link to={`/${initId(product.name)}-id,${product._id}`} className={classNameBlock}>
      <img className='mx-auto max-h-[186px]' src={product.image} alt='img' />
      <div className='p-2'>
        ...
        <div className='flex'>
          <Rate className='text-[10px]' disabled allowHalf defaultValue={rating} />
